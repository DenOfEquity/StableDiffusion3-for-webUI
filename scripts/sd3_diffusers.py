...
from diffusers import StableDiffusion3Pipeline, StableDiffusion3Img2ImgPipeline#, SD3Transformer2DModel
...
def predict(positive_prompt, negative_prompt, width, height, guidance_scale, clipskip, 
    ...
    if SD3Storage.triple == True:
        split_positive = positive_prompt.split('|')
        split_negative = negative_prompt.split('|')

        c = len(split_positive)
        if c == 2 and SD3Storage.T5 == False:
...
# 3 positives, 3 negatives (1 of each necessary)
    ...
    useCachedEmbeds = (SD3Storage.lastPositive == positive_prompt and
                       SD3Storage.lastNegative == negative_prompt and
    ...
    if useCachedEmbeds:
        ...
        with torch.no_grad():
            ...
            if SD3Storage.T5 == True:
                ...
                #4096 is transformer joint_attention_dim
                positive_embeds_3 = torch.zeros((num_images, 255, 4096),
                                                device='cpu', dtype=torch.float16, )
                negative_embeds_3 = torch.zeros((num_images, 255, 4096),
                                                device='cpu', dtype=torch.float16, )
                #end: T5
        ...
        SD3Storage.lastPositive = positive_prompt
        SD3Storage.lastNegative = negative_prompt
        SD3Storage.clipskip = clipskip
...
def on_ui_tabs():
    ...
    def toggle3 ():
        ...
        if SD3Storage.triple == False:
            SD3Storage.triple = True
            return gr.Button.update(variant='primary')
        else:
            SD3Storage.triple = False
...
##                        label='Sampler', value="default", type='value', scale=0)
                    triple = ToolButton(value="3", variant='secondary', tooltip="use triple prompt")
...
#   caption not displaying linebreaks, alt text does
        ...
        triple.click(toggle3, inputs=[], outputs=triple)
